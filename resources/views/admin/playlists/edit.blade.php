...
@section('title', 'Edit Playlist - Admin')
...
<div class="min-h-screen bg-gradient-to-b from-black via-blue-950 to-black">
    <div class="container mx-auto px-4 py-12">
        <div class="max-w-4xl mx-auto">
            <div class="mb-8">
                <h1 class="text-4xl font-bold text-white mb-2">Edit Playlist</h1>
                <p class="text-zinc-400">Update playlist information</p>
            </div>
            ...
            <form action="{{ route('admin.playlists.update', $playlist->UUID) }}" method="POST" enctype="multipart/form-data" class="bg-zinc-900 rounded-lg p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-white mb-2">Current Cover Image</label>
                        <div class="mb-4">
                            <img 
                    ...
                    <div>
                        <label class="block text-sm font-medium text-white mb-2">Songs in This Playlist</label>
                        <div class="max-h-64 overflow-y-auto bg-zinc-800 border border-zinc-700 rounded-lg p-4 mb-4">
                            <div class="space-y-2">
                                @foreach($playlist->songs as $playlistSong)
                                <div class="flex items-center justify-between gap-3 p-3 bg-zinc-700 rounded-lg">
                                    <div class="flex items-center gap-3 flex-1">
                                        <input 
                                            type="checkbox" 
                                            name="songs[]" 
                                            value="{{ $playlistSong->IndirimboID }}"
                                            checked
                                            class="w-5 h-5 text-green-500 bg-zinc-800 border-zinc-700 rounded focus:ring-green-500"
                                        >
                                        <div class="flex-1">
                                            <div class="text-white font-medium">{{ $playlistSong->IndirimboName }}</div>
                                            @if($playlistSong->artist)
                                                <div class="text-sm text-zinc-400">{{ $playlistSong->artist->StageName }}</div>
                                            @endif
                                        </div>
                                    </div>
                                    <button 
                                        type="button" 
                                        onclick="removeSongFromPlaylist(this, '{{ $playlistSong->IndirimboID }}')"
                        ...
                        <p class="text-xs text-zinc-500 mb-4">Uncheck a song or click "Remove" to remove it from this playlist. This does not delete the song.</p>
                    </div>
                    @endif
                            ...
                            @if($allSongs->count() > 0)
                                <div class="space-y-2">
                                    @foreach($allSongs as $song)
                                        @if(!in_array($song->IndirimboID, $playlistSongIds))
                                ...
                                @if($allSongs->whereNotIn('IndirimboID', $playlistSongIds)->count() === 0)
                                    <p class="text-zinc-400 text-center py-4">All available songs are already in this playlist.</p>
                                @endif
                            @else
                                <p class="text-zinc-400 text-center py-4">No songs available. Create some songs first.</p>
                            @endif
                        </div>
                        <p class="text-xs text-zinc-500 mt-2">Check songs to add them to this playlist</p>
                    </div>
                    ...
                    <div>
                        <label class="flex items-center gap-3">
                            <input 
                                ...
                                name="IsFeatured" 
                                value="1"
                                {{ old('IsFeatured', $playlist->IsFeatured) ? 'checked' : '' }}
                                class="w-5 h-5 text-green-500 bg-zinc-800 border-zinc-700 rounded focus:ring-green-500"
                            >
...
<script>
function removeSongFromPlaylist(button, songId) {
