...
@section('title', 'Edit Orchestra - Admin')
...
<div class="min-h-screen bg-gradient-to-b from-black via-blue-950 to-black">
    <div class="container mx-auto px-4 py-12">
        <div class="max-w-4xl mx-auto">
            <div class="mb-8">
                <h1 class="text-4xl font-bold text-white mb-2">Edit Orchestra</h1>
                <p class="text-zinc-400">Update orchestra information</p>
            </div>
            ...
            <form action="{{ route('admin.orchestras.update', $orchestra->UUID) }}" method="POST" enctype="multipart/form-data" class="bg-zinc-900 rounded-lg p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            id="OrchestreName" 
                            name="OrchestreName" 
                            value="{{ old('OrchestreName', $orchestra->OrchestreName) }}"
                            required
                    ...
                    <div>
                        ...
                        >{{ old('Description', $orchestra->Description) }}</textarea>
                    </div>
                    ...
                    <div>
                        <label class="flex items-center gap-3">
                            <input 
                                ...
                                name="IsFeatured" 
                                value="1"
                                {{ old('IsFeatured', $orchestra->IsFeatured) ? 'checked' : '' }}
                    ...
                    <!-- Current Orchestra Songs -->
                    @if($orchestra->songs->count() > 0)
                    <div>
                        <label class="block text-sm font-medium text-white mb-2">Songs by {{ $orchestra->OrchestreName }}</label>
                        <div class="max-h-64 overflow-y-auto bg-zinc-800 border border-zinc-700 rounded-lg p-4 mb-4">
                            <div class="space-y-2">
                                @foreach($orchestra->songs as $orchestraSong)
                                <div class="flex items-center justify-between gap-3 p-3 bg-zinc-700 rounded-lg">
                                    <div class="flex items-center gap-3 flex-1">
                                        <input 
                                            type="checkbox" 
                                            name="songs[]" 
                                            value="{{ $orchestraSong->IndirimboID }}"
                                            checked
                                            class="w-5 h-5 text-green-500 bg-zinc-800 border-zinc-700 rounded focus:ring-green-500"
                                        >
                                        <div class="flex-1">
                                            <div class="text-white font-medium">{{ $orchestraSong->IndirimboName }}</div>
                                            @if($orchestraSong->artist)
                                                <div class="text-sm text-zinc-400">{{ $orchestraSong->artist->StageName }}</div>
                                            @endif
                                        </div>
                                    </div>
                                    <button 
                                        type="button" 
                                        onclick="removeSongFromOrchestra(this, '{{ $orchestraSong->IndirimboID }}')"
                        ...
                        <p class="text-xs text-zinc-500 mb-4">Uncheck a song or click "Remove" to remove it from this orchestra. This does not delete the song.</p>
                    </div>
                    @endif
                    ...
                    <div class="mt-6">
                        <div class="bg-zinc-900 rounded-lg p-8 border border-zinc-700">
                            <h3 class="text-xl font-semibold text-white mb-4">Upload New Songs for {{ $orchestra->OrchestreName }}</h3>
                            
                            <form action="{{ route('admin.orchestras.songs.store', $orchestra->UUID) }}" method="POST" enctype="multipart/form-data" id="upload-form">
                                @csrf
                                <input type="hidden" name="OrchestreID" value="{{ $orchestra->OrchestreID }}">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-zinc-400 mb-3">Select existing songs to add to this orchestra</p>
                        <div class="max-h-96 overflow-y-auto bg-zinc-800 border border-zinc-700 rounded-lg p-4">
                            @if($allSongs->count() > 0)
                                <div class="space-y-2">
                                    @foreach($allSongs as $song)
                                        @if(!in_array($song->IndirimboID, $orchestraSongIds))
                                ...
                                @if($allSongs->whereNotIn('IndirimboID', $orchestraSongIds)->count() === 0)
                                    <p class="text-zinc-400 text-center py-4">All available songs are already assigned to this orchestra.</p>
                                @endif
                            @else
                                <p class="text-zinc-400 text-center py-4">No songs available. Create some songs first.</p>
                            @endif
                        </div>
                        <p class="text-xs text-zinc-500 mt-2">Check songs to add them to this orchestra</p>
                    </div>
...
function removeSongFromOrchestra(button, songId) {
