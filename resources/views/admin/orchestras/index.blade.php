...
<div class="min-h-screen bg-gradient-to-b from-black via-blue-950 to-black">
    <div class="container mx-auto px-4 py-12">
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-4xl font-bold text-white mb-2">Orchestras</h1>
                <p class="text-zinc-400">Manage all orchestras in the Karahanyuze collection</p>
            </div>
            <a 
                href="{{ route('admin.orchestras.create') }}" 
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-zinc-800">
                    ...
                    <tr class="hover:bg-zinc-800/50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="text-white">{{ $orchestra->songs_count ?? 0 }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($orchestra->IsFeatured)
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex items-center gap-3">
                                <a 
                                    href="{{ route('admin.orchestras.edit', $orchestra->UUID) }}" 
                                ...
                                <button 
                                    type="button"
                                    onclick="showDeleteOrchestraWarning('{{ $orchestra->UUID }}', '{{ addslashes($orchestra->OrchestreName) }}');"
...
<!-- Delete Orchestra Warning Modal -->
<div id="deleteOrchestraWarningModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center">
    <div class="bg-zinc-800 rounded-lg p-6 max-w-md w-full mx-4">
        <div class="flex items-start gap-4 mb-4">
            ...
            <div class="flex-1">
                <h3 class="text-xl font-bold text-white mb-2">Delete Orchestra</h3>
                <p class="text-zinc-300 mb-4">Are you sure you want to delete this orchestra? This action cannot be undone.</p>
                <div class="bg-red-500/10 border border-red-500/50 rounded-lg p-3 mb-4">
                    <p class="text-red-300 text-sm font-medium mb-1">Orchestra Name:</p>
                    <p class="text-white text-sm" id="deleteOrchestraName"></p>
                </div>
                ...
                <form id="deleteOrchestraForm" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    <div class="flex items-center gap-3">
                        ...
                        <button type="button" onclick="closeDeleteOrchestraWarning()" class="px-6 py-3 bg-zinc-700 hover:bg-zinc-600 text-white font-semibold rounded-lg transition-colors">
                            Cancel
                        </button>
                    </div>
                </form>
...
<script>
function showDeleteOrchestraWarning(uuid, orchestraName) {
    document.getElementById('deleteOrchestraName').textContent = orchestraName;
    document.getElementById('deleteOrchestraForm').action = '{{ url("/admin/orchestre") }}/' + uuid;
    document.getElementById('deleteOrchestraWarningModal').classList.remove('hidden');
    document.getElementById('deleteOrchestraWarningModal').classList.add('flex');
    document.body.style.overflow = 'hidden';
}

function closeDeleteOrchestraWarning() {
    document.getElementById('deleteOrchestraWarningModal').classList.add('hidden');
    document.getElementById('deleteOrchestraWarningModal').classList.remove('flex');
    document.body.style.overflow = '';
}

document.getElementById('deleteOrchestraWarningModal')?.addEventListener('click', function(e) {
    if (e.target === this) {
        closeDeleteOrchestraWarning();
    }
});
</script>
@endsection
