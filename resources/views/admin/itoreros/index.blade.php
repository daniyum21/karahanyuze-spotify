...
<!-- Delete Itorero Warning Modal -->
<div id="deleteItoreroWarningModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center">
    <div class="bg-zinc-800 rounded-lg p-6 max-w-md w-full mx-4">
        <div class="flex items-start gap-4 mb-4">
            ...
            <div class="flex-1">
                <h3 class="text-xl font-bold text-white mb-2">Delete Itorero</h3>
                <p class="text-zinc-300 mb-4">Are you sure you want to delete this itorero? This action cannot be undone.</p>
                <div class="bg-red-500/10 border border-red-500/50 rounded-lg p-3 mb-4">
                    <p class="text-red-300 text-sm font-medium mb-1">Itorero Name:</p>
                    <p class="text-white text-sm" id="deleteItoreroName"></p>
                </div>
                ...
                <form id="deleteItoreroForm" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    <div class="flex items-center gap-3">
                        ...
                        <button type="button" onclick="closeDeleteItoreroWarning()" class="px-6 py-3 bg-zinc-700 hover:bg-zinc-600 text-white font-semibold rounded-lg transition-colors">
                            Cancel
                        </button>
                    </div>
                </form>
...
function showDeleteItoreroWarning(uuid, itoreroName) {
    document.getElementById('deleteItoreroName').textContent = itoreroName;
    document.getElementById('deleteItoreroForm').action = '{{ url("/admin/amatorero") }}/' + uuid;
    document.getElementById('deleteItoreroWarningModal').classList.remove('hidden');
    document.getElementById('deleteItoreroWarningModal').classList.add('flex');
    document.body.style.overflow = 'hidden';
}

function closeDeleteItoreroWarning() {
    document.getElementById('deleteItoreroWarningModal').classList.add('hidden');
    document.getElementById('deleteItoreroWarningModal').classList.remove('flex');
    document.body.style.overflow = '';
}

document.getElementById('deleteItoreroWarningModal')?.addEventListener('click', function(e) {
    if (e.target === this) {
