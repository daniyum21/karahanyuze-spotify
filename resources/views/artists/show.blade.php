    ...
    <div class="relative z-10 px-6 pt-8 pb-24">
        ...
        <div class="flex items-end gap-6 mb-8">
            <img 
                ...
                class="w-[232px] h-[232px] rounded-full object-cover shadow-2xl flex-shrink-0"
            />
            <div class="flex-1 pb-4">
                <p class="text-sm text-white mb-2">Artist</p>
                <h1 class="text-5xl md:text-7xl font-bold text-white mb-4">{{ $artist->StageName }}</h1>
                
                @if($artist->FirstName || $artist->LastName)
                <p class="text-white text-lg mb-2">{{ $artist->FirstName }} {{ $artist->LastName }}</p>
                @endif

                <div class="flex items-center gap-2 text-sm text-zinc-300">
                    <span>{{ $artist->songs->count() }} songs</span>
                </div>
            </div>
        </div>
            ...
            @php
                ...
                if (Auth::check()) {
                    $isFavorited = Auth::user()->favorites()
                        ->where('FavoriteType', 'Artist')
                        ->where('FavoriteID', $artist->UmuhanziID)
                        ->exists();
                }
            @endphp
            <x-like-button 
                entity-type="artist" 
                entity-id="{{ $artist->UmuhanziID }}" 
                ...
                >
                    ...
                    <div class="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-zinc-400 text-sm truncate">{{ $artist->StageName }}</p>
                        </div>
                    </div>
        ...
        <div class="bg-zinc-900/50 rounded-lg p-6 mb-8">
            ...
            <div class="text-zinc-300 whitespace-pre-wrap">{{ strip_tags($artist->Description) }}</div>
        </div>
        @endif
    </div>
</div>
