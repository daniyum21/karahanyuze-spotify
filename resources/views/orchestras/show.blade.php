...
@section('title', $orchestra->OrchestreName . ' - Karahanyuze')
...
<div class="min-h-screen bg-gradient-to-b from-black via-blue-950 to-black pb-24">
    <div class="container mx-auto px-4 py-8">
        <!-- Back Button -->
        <div class="mb-6">
            <a href="{{ route('orchestre.index') }}" class="inline-flex items-center gap-2 text-white hover:text-green-500 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                </svg>
                <span>Back to Orchestre</span>
            </a>
        </div>
        ...
        <div class="flex flex-col md:flex-row gap-8 mb-12">
            ...
            <div class="flex-1">
                <h1 class="text-4xl md:text-5xl font-bold text-white mb-4">{{ $orchestra->OrchestreName }}</h1>

                @if($orchestra->Description)
                <p class="text-zinc-300 mb-6 leading-relaxed">{{ $orchestra->Description }}</p>
                @endif

                <div class="flex flex-wrap gap-6 text-sm text-zinc-400 mb-6">
                    <div>
                        <span class="font-semibold text-zinc-500">Songs:</span> 
                        <span class="text-white">{{ $orchestra->songs->count() }}</span>
                    </div>
                </div>
                ...
                @php
                    ...
                    if (Auth::check()) {
                        $isFavorited = Auth::user()->favorites()
                            ->where('FavoriteType', 'Orchestra')
                            ->where('FavoriteID', $orchestra->OrchestreID)
                            ->exists();
                    }
                @endphp
                <button 
        ...
        <!-- Songs Section -->
        @if($orchestra->songs && $orchestra->songs->count() > 0)
        @php
            $songCount = $orchestra->songs->count();
        ...
        <div class="mb-8">
            ...
            <!-- Songs List -->
            <div class="space-y-2">
                @foreach($orchestra->songs as $index => $song)
                @php
                    $songYear = $song->created_at ? $song->created_at->format('Y') : 'N/A';
                    $artistName = $song->artist ? $song->artist->StageName : ($song->itorero ? $song->itorero->ItoreroName : 'Unknown');
                @endphp
        ...
        <div class="text-center py-12">
            <p class="text-zinc-400 text-lg">No songs available for this orchestra.</p>
        </div>
        @endif
    </div>
</div>
...
@if($orchestra->songs && $orchestra->songs->count() > 0)
@php
    $songList = $orchestra->songs->map(function($song) {
        return [
            'id' => $song->IndirimboID,
...
@endphp
<script>
    // Store the song list for this orchestra page
    ...
    function toggleFavorite() {
        ...
        const orchestraId = {{ $orchestra->OrchestreID }};
        
        // Send AJAX request to toggle favorite
        fetch(`/favorites/orchestra/${orchestraId}/toggle`, {
            method: 'POST',
            headers: {
    ...
    // Auto-play first song when orchestra page loads
