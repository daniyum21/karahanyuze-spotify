    ...
    <div class="container mx-auto px-4 py-12">
        <div class="max-w-4xl mx-auto">
            <div class="mb-8">
                ...
                <p class="text-zinc-400">Add a new song to the Karahanyuze collection. Your song will be reviewed and approved by an administrator before being published.</p>
            </div>
            ...
            <form action="{{ route('user.songs.store') }}" method="POST" enctype="multipart/form-data" class="bg-zinc-900 rounded-lg p-8">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    @foreach($artists as $artist)
                                        <option value="{{ $artist->UmuhanziID }}" {{ (old('UmuhanziID') == $artist->UmuhanziID || ($selectedArtist && $selectedArtist->UmuhanziID == $artist->UmuhanziID)) ? 'selected' : '' }}>
                                            {{ $artist->StageName }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    @foreach($orchestras as $orchestra)
                                        <option value="{{ $orchestra->OrchestreID }}" {{ (old('OrchestreID') == $orchestra->OrchestreID || ($selectedOrchestra && $selectedOrchestra->OrchestreID == $orchestra->OrchestreID)) ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    @foreach($itoreros as $itorero)
                                        <option value="{{ $itorero->ItoreroID }}" {{ (old('ItoreroID') == $itorero->ItoreroID || ($selectedItorero && $selectedItorero->ItoreroID == $itorero->ItoreroID)) ? 'selected' : '' }}>
                                            {{ $itorero->ItoreroName }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <p class="text-xs text-zinc-500 mt-2">
                            ...
                            <a href="{{ route('user.artists.create') }}" class="text-green-500 hover:text-green-400 underline">Create a new artist</a>, 
                            <a href="{{ route('user.orchestras.create') }}" class="text-green-500 hover:text-green-400 underline">create an orchestra</a>, or 
                            <a href="{{ route('user.itoreros.create') }}" class="text-green-500 hover:text-green-400 underline">create an itorero</a> first.
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        >
                        <p class="text-xs text-zinc-500 mt-2">Maximum file size: 50MB (required)</p>
                    </div>
                    ...
                    <!-- Info Box -->
                    <div class="bg-blue-500/20 border border-blue-500 text-blue-400 px-4 py-3 rounded-lg">
                        <p class="text-sm">
                            <strong>Note:</strong> Your song will be submitted for review. An administrator will review and approve it before it's published on the site.
                        </p>
                    </div>
